...
export default function Chat() {
    ...
    // Visual Logic (Cloaking & Loading)
    const [isRoomReady, setIsRoomReady] = useState(false);
    const [pendingImages, setPendingImages] = useState(0); 
    const imagesLoadedCount = useRef(0);

    // Inputs & Singularity Engine
    const [msgText, setMsgText] = useState('');
    const [imgFile, setImgFile] = useState(null);
    const [blackHoleSize, setBlackHoleSize] = useState(0); // 0 to 100%
    ...
    const lastLoadedTeamId = useRef(null); // Prevents draft race conditions
    ...
    // Navigation Logic (History API / Swipe Back)
    useEffect(() => {
        const handlePopState = (event) => {
            // Null state means we are back at the lobby
            if (event.state === null) {
                setCurrentTeam(null);
                setMessages([]);
            } 
            // If state has teamId, we (theoretically) restore that room
            else if (event.state?.teamId) {
                const team = myTeamsRef.current.find(t => t.$id === event.state.teamId);
    ...
    // ==========================================
    // 4. TELEMETRY CACHING (Drafts & Resize)
    // ==========================================
    ...
    // ==========================================
    // 5. VIEWPORT LOGIC (Room Entry)
    // ==========================================
    useEffect(() => {
        if (currentTeam) {
            setMessages([]); 
            setIsRoomReady(false); // CLOAKING ENGAGED
    ...
    useEffect(() => {
        ...
        if (!isRoomReady) {
            ...
            const timer = setTimeout(finalizeRoomView, 2000); // Fallback
    ...
    const finalizeRoomView = () => {
        ...
        requestAnimationFrame(() => {
            requestAnimationFrame(() => setIsRoomReady(true)); // CLOAKING DISENGAGED
        });
    };
    ...
    const handleInput = (e) => {
        ...
        setMsgText(val);
        // Resize/Save handled by useLayoutEffect
    ...
    // ==========================================
    // 7. USER ACTIONS (Auth / Send / Context)
    // ==========================================
